...
###############################################################################
TEMP_FILE_DIR = 'temp_1_6_vs_2_6/'
file_name =
TEMP_FILE_NAME = TEMP_FILE_DIR + file_name
...
def allocate(target_allocations, anti_target_allocations, is_target_choices):
    ...
    # In first trial, create a file with the allocations per trial
    if trial ==0:
      rewards = {}
      rewards_in_1_6 = round(100/6)
      rewards_in_2_6 = round(2*100/6)
      rewards['1_6'] = np.random.permutation(np.concatenate((np.ones(rewards_in_1_6),np.zeros(100-rewards_in_1_6))))
      rewards['2_6'] = np.random.permutation(np.concatenate((np.ones(rewards_in_2_6),np.zeros(100-rewards_in_2_6))))
      with open(TEMP_FILE_NAME, 'wb') as output_file:
        pickle.dump(rewards, output_file, pickle.HIGHEST_PROTOCOL)

    # Extact from the file the allocation sequences and use them to allocate in
    # current trial
    with open(TEMP_FILE_NAME, 'rb') as output_file:
      rewards = pickle.load(output_file)
    target_alternative, anti_target_alternative = rewards['2_6'][trial], rewards['1_6'][trial]
...
if __name__ == '__main__':
    ...
    allocation = allocate(*parsed_input)
    output(*allocation)
